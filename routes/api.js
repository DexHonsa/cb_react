...
var URL =
	"mongodb://iwantmoredexx:Awesomeo21!@cluster0-shard-00-00-l9gyz.mongodb.net:27017,cluster0-shard-00-01-l9gyz.mongodb.net:27017,cluster0-shard-00-02-l9gyz.mongodb.net:27017/commonbrain?ssl=true&replicaSet=Cluster0-shard-0&authSource=admin";
		...
		workbook.xlsx.readFile(req.file.path).then(function() {
			...
			for (var i = 0, len = sheet._rows.length; i < len; i++) {
				if (sheet._rows[i]._cells != null) {
					for (
						var i2 = 0, len2 = sheet._rows[i]._cells.length;
						i2 < len2;
						i2++
					) {
						if (sheet._rows[i]._cells[i2] != null) {
							if (
								sheet._rows[i]._cells[i2]._value.model.style != null &&
								sheet._rows[i]._cells[i2]._value.model.style
									.toString()
									.indexOf("~") > -1
							) {
								var styleArr = sheet._rows[i]._cells[i2]._value.model.style
									.toString()
									.split("~");
								var styleLetter;
								...
								var getCell = worksheet.getCell(
									styleLetter + (parseInt(styleArr[2]) + 1)
								).style;
								if (getCell.numFmt != null) {
									var tempNumFormat = JSON.parse(
										CircularJSON.stringify(getCell.numFmt)
									);
							...
							if (
								sheet._rows[i]._cells[i2]._value.model.style != null &&
								sheet._rows[i]._cells[i2]._value.model.style.numFmt != null
							) {
								if (
									sheet._rows[i]._cells[i2]._value.model.style.numFmt.indexOf(
										"$"
									) > -1
								) {
								}
		...
		workbook.xlsx.readFile(req.file.path).then(function() {
			...
			for (var i = 7; i <= 22; i++) {
				for (var ic = 0, len = sheet._rows[i]._cells.length; ic <= len; ic++) {
					...
					if (
						sheet._rows[i]._cells[ic] !== undefined &&
						sheet._rows[i]._cells[ic] !== null
					) {
						var header;

						if (
							sheet._rows[i]._cells[ic]._value.model.address.indexOf("A") !== -1
						) {
							header = sheet._rows[i]._cells[ic]._value.model.value;
						}
						if (
							sheet._rows[i]._cells[ic]._value.model.address.indexOf("B") !== -1
						) {
						...
						if (
							sheet._rows[i]._cells[ic]._value.model.address.indexOf("C") !== -1
						) {
							if (sheet._rows[i]._cells[ic]._value.model.value != undefined) {
						...
						if (
							sheet._rows[i]._cells[ic]._value.model.address.indexOf("D") !== -1
						) {
							if (sheet._rows[i]._cells[ic]._value.model.value != undefined) {
		...
		workbook.xlsx.readFile(req.file.path).then(function() {
			...
			for (var i = 0, len = sheet2._rows.length; i < len; i++) {
				if (sheet2._rows[i]._number != 1) {
					var Obj = {};
					for (
						var i2 = 0, len2 = sheet2._rows[i]._cells.length;
						i2 < len2;
						i2++
					) {
						if (
							sheet2._rows[i]._cells[i2] !== null &&
							sheet2._rows[i]._cells[i2] !== undefined
						) {
							var cell = sheet2._rows[i]._cells[i2]._value.model.address;
							if (sheet2._rows[i]._cells[i2]._value.model.value === undefined) {
								if (cell.indexOf("A") > -1) {
									Obj["applicationName"] =
										sheet2._rows[i]._cells[i2]._value.model.text;
								}
								if (cell.indexOf("B") > -1) {
									Obj["applicationType"] =
										sheet2._rows[i]._cells[i2]._value.model.text;
								...
								if (cell.indexOf("J") > -1) {
									Obj["compliancyPercentage"] =
										sheet2._rows[i]._cells[i2]._value.model.text;
								}
								if (cell.indexOf("K") > -1) {
									Obj["compliancyPercentageLink"] =
										sheet2._rows[i]._cells[i2]._value.model.text;
								}
								if (cell.indexOf("L") > -1) {
									Obj["downloadLink"] =
								...
								if (cell.indexOf("A") > -1) {
									Obj["applicationName"] =
										sheet2._rows[i]._cells[i2]._value.model.value;
								}
								if (cell.indexOf("B") > -1) {
									Obj["applicationType"] =
										sheet2._rows[i]._cells[i2]._value.model.value;
...
exports.ImportBasic = function(req, res) {
	...
	upload(req, res, function(err) {
		...
		for (var i = 0, len = nameRangesRaw.length; i < len; i++) {
			...
			if (
				nameRangesRaw[i].Name != null &&
				nameRangesRaw[i].Name == "CBrainBarChart"
			) {
				var refArr = nameRangesRaw[i].Ref.split("$");
		...
		for (
			...
			if (i > 1) {
				...
				//
				if (
					sheet.Sheets[MainSheet][Object.keys(sheet.Sheets[MainSheet])[i]].w !=
					null
				) {
					...
					MongoClient.connect(URL, function(err, db) {
						...
						collection.insert(portfolioItemObj, function(err, result) {
							...
							var basicDetailObj = {
								...
								barChart: nameRanges.CBrainBarChart
							};
			...
			function(err) {
				...
				for (
					...
					if (i > 1) {
						...
						if (
							sheet.Sheets[MainSheet][Object.keys(sheet.Sheets[MainSheet])[i]]
								.w != null
						) {
							rows[rowNumber][Object.keys(sheet.Sheets[MainSheet])[i]] =
								sheet.Sheets[MainSheet][
				...
				MongoClient.connect(URL, function(err, db) {
					...
					collection.remove({ portfolioItemId: portfolioItemId });
					portfolioItems.update(
						{ _id: ObjectId(portfolioItemId) },
						...
						collection.insert(portfolioItemObj, function(err, result) {
							...
							workbook.xlsx
								...
								.then(function() {
									...
									var headers = [];
									for (var i = 0, len = sheet2._rows.length; i < len; i++) {
										//cycle through sheet rows

										var rowObj = {
											userId: userId,
											portfolioId: portfolioId,
											portfolioItemId: portfolioItemId
										}; //create row object for data
										if (sheet2._rows[i]._cells != null) {
											//cycle through cells in each row
											for (
												var i2 = 0, len2 = sheet2._rows[i]._cells.length;
												i2 < len2;
												i2++
												...
												if (sheet2._rows[i]._cells[i2] != null) {
													var cell =
														sheet2._rows[i]._cells[i2]._value.model.address;
													if (
														cell.indexOf(headerRow) != -1 &&
														cell.length == 2
													) {
														headers.push(
															sheet2._rows[i]._cells[i2]._value.model.value
													...
													if (
														...
														if (
															sheet2._rows[i]._cells[i2]._value.model.result ===
															undefined
														) {
															rowObj[header] = "--";
														} else {
														...
														//end
														if (
															sheet2._rows[i]._cells[i2]._value.model.style !=
																null &&
															sheet2._rows[i]._cells[i2]._value.model.style
																.numFmt != null
														) {
															...
															if (numFormat.indexOf("$") > -1) {
																rowObj[header] =
																	"$" +
																	sheet2._rows[i]._cells[
																		i2
																	]._value.model.value.formatMoney(2);
															}
															if (numFormat.indexOf("%") > -1) {
																rowObj[header] =
																	"%" +
																	Math.round(
																		sheet2._rows[i]._cells[i2]._value.model
																			.value * 100
																	);
															}
															if (
														...
														if (
															...
															var styleArr = sheet2._rows[i]._cells[
																i2
															]._value.model.style
																.toString()
																.split("~");
									...
									var basicSheet2 = JSON.parse(
										CircularJSON.stringify(basicSheet)
									);
									var basicDetailObj = {
										userId: userId,
										portfolioId: portfolioId,
									...
									for (
										var i = 0, len = basicSheet2._rows.length;
										i < len;
										i++
									) {
										if (basicSheet2._rows[i]._cells != null) {
											//cycle through cells in each row
											for (
												var i2 = 0, len2 = basicSheet2._rows[i]._cells.length;
												i2 < len2;
												i2++
											) {
												var cell =
													basicSheet2._rows[i]._cells[i2]._value.model.address;

												if (cell == "A2") {
													basicDetailObj["title"] =
														basicSheet2._rows[i]._cells[i2]._value.model.value;
												}
												if (cell == "B2") {
													basicDetailObj["imgUrl"] =
														basicSheet2._rows[i]._cells[i2]._value.model.value;
												}
											}
										}
								...
								for (
									...
									if (sheet2._rows[i]._cells[i2] != null) {
										...
										if (
											...
											//end
											if (
												sheet2._rows[i]._cells[i2]._value.model.style != null &&
												sheet2._rows[i]._cells[i2]._value.model.style.numFmt !=
													null
											) {
												var numFormat =
													sheet2._rows[i]._cells[i2]._value.model.style.numFmt;
				...
				if (sheet2._rows[i]._cells != null) {
					...
					for (
						...
						if (sheet2._rows[i]._cells[i2] != null) {
							...
							if (sheet2._rows[i]._cells[i2]._value.model.value === undefined) {
								if (
									sheet2._rows[i]._cells[i2]._value.model.result === undefined
								) {
									rowObj[header] = "--";
								} else {
									rowObj[header] =
								...
								if (
									sheet2._rows[i]._cells[i2]._value.model.style != null &&
									sheet2._rows[i]._cells[i2]._value.model.style.numFmt != null
								) {
									var numFormat =
										sheet2._rows[i]._cells[i2]._value.model.style.numFmt;
									if (numFormat.indexOf("$") > -1) {
										rowObj[header] =
											"$" +
											sheet2._rows[i]._cells[i2]._value.model.value.formatMoney(
												2
											);
									}
									if (numFormat.indexOf("%") > -1) {
										...
										rowObj[header] =
											"%" +
											Math.round(
												sheet2._rows[i]._cells[i2]._value.model.value * 100
											);
									}
									if (
										numFormat.indexOf("$") < -1 &&
								...
								if (
									...
									var styleArr = sheet2._rows[i]._cells[i2]._value.model.style
										.toString()
										.split("~");
									var styleLetter;
	...
	upload(req, res, function(err) {
		workbook.xlsx.readFile(req.file.path).then(function() {
			...
			for (var i = 0, len = sheet2._rows.length; i < len; i++) {
				...
				if (
					sheet2._rows[i]._cells !== null &&
					sheet2._rows[i]._cells !== undefined
				) {
					//cycle through cells in each row
					for (
						var i2 = 0, len2 = sheet2._rows[i]._cells.length;
...
exports.GetPropertyInfo = function(req, res) {
	...
	MongoClient.connect(URL, function(err, db) {
		...
		collection
			.find({ portfolioItemId: portfolioItemId })
			.toArray(function(err, result) {
				if (err) throw err;
				res.send(result);
				db.close();
...
exports.GetHeaders = function(req, res) {
	...
	MongoClient.connect(URL, function(err, db) {
		...
		collection
			.find({ portfolioItemId: portfolioItemId, "Sheet Name": sheetName })
			.toArray(function(err, result) {
				var headerArr = [];

...
class Main extends React.Component {
	...
	getStuff() {
		...
		axios
			.post("/api/getFilename", { portfolioItemId: this.state.portfolioItemId })
			.then(
				res => {
		...
		if (this.state.uploadPopup) {
			uploadPopup = (
				<UploadData
					...
					portfolioItemId={this.state.portfolioItemId}
					filename={this.state.filename}
					hidePopup={this.showUploadPopup.bind(this)}
					closePopup={this.closeUploadPopup.bind(this)}
				/>
			);
		} else {
			uploadPopup = null;
			...
			<div>
				...
				<div className="side-stage">
					{this.state.topTitleVisible ? (
						<div
							className="fixed-address-bar animated "
					...
					<div className="property-top-container">
						<div
							...
							style={
								!this.state.isLoading
									? {
											backgroundImage:
												"url(" + this.state.propertyInfo[0].imgUrl + ")"
										}
									: {}
							}>
								...
								this.state.tabs.map(function(data, i) {
									if (data !== undefined) {
										return (
											<div
												key={i}
												onClick={() => this.activateTab(i)}
												className={
													this.state.activeTabs[i]
						...
						{!this.state.isLoading && (
							<TabBlock
								...
								activeSideTabName={this.props.activeSideTabName}
								tabName={this.state.activeTabName}
								portfolioId={this.state.portfolioId}
								portfolioItemId={this.state.portfolioItemId}
								barChart={this.state.propertyInfo[0].barChart}
							/>
						)}
					</div>

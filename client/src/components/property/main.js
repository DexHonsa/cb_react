...
class Main extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    axios.post("/api/getHeaders/", data).then(res => {
      that.setState({headers: res.data, headersLoaded: true});
    }, err => {});
    axios.post("/api/getTabs/", data).then(res => {
  ...
  getStuff() {
    ...
    axios.post("/api/getFilename", {portfolioItemId: this.state.portfolioItemId}).then(res => {
      that.setState({filename: res.data.name});
    }, err => {});
    axios.post("/api/getPropertyInfo/", {
      responseType: "arraybuffer",
      userId: this.props.auth.user.id,
      portfolioItemId: this.state.portfolioItemId
    }).then(res => {
      that.setState({propertyInfo: res.data, isLoading: false});
      console.log(that.state.propertyInfo);
      if (that.state.propertyInfo[0].imgUrl.indexOf("http") < 0) {
        console.log("HEEYYY");
        var propInfo = that.state.propertyInfo;
        propInfo[0].imgUrl = "http://" + that.state.propertyInfo[0].imgUrl;

        that.setState({propertyInfo: propInfo});
      } else {}
    });
  }
  downloadExcel() {
    var that = this;
    axios.post("/api/downloadNewExcel/", {
      userId: this.props.auth.user.id,
      portfolioId: this.state.portfolioId,
      filename: this.state.filename
    }, {responseType: "arraybuffer"}).then(res => {
      FileDownload(res.data, that.state.filename);
    }, err => {});
  }
  showUploadPopup() {
    this.setState({
  ...
  render() {
    ...
    return (<div>
      ...
      <div className="side-stage">
        {
          this.state.topTitleVisible
            ? (<div className="fixed-address-bar animated " style={{
                display: "none",
                opacity: 0
              }}>
              <div className="container" style={{
                ...
                {this.state.propertyInfo[0].title}
              </div>
            </div>)
            : null
        }
        <div className="property-top-container">
          <div className="property-img" style={!this.state.isLoading
              ? {
                backgroundImage: "url(" + this.state.propertyInfo[0].imgUrl + ")"
              }
              : {}}>
            <div className="property-main-title">
              {!this.state.isLoading && this.state.propertyInfo[0].title}
              <div className="side-stage-top">
                <div className="download-btn" data-tip="Upload Excel" style={{
                    right: "160px"
                  }} onClick={this.showUploadPopup.bind(this)}>
                  <i className="fa fa-upload"/>
                </div>
                <div className="download-btn" data-tip="Download Excel" onClick={this.downloadExcel.bind(this)}>
          ...
          <div className="property-tab-container">
            {
              this.state.tabsLoaded && this.state.tabs.map(function(data, i) {
                if (data !== undefined) {
                  return (<div key={i} onClick={() => this.activateTab(i)} className={this.state.activeTabs[i]
